...
namespace Checkers.UI
{
    ...
    public class CheckersUI : System.Windows.Forms.UserControl
    {
        ...
        public Color BoardBackColor
        {
            get
            {
                return boardBackColor;
            }
            set
            {
                boardBackColor = value;
                Refresh();
            }
        }
        ...
        public Color BoardForeColor
        {
            get
            {
                return boardForeColor;
            }
            set
            {
                boardForeColor = value;
                Refresh();
            }
        }
        ...
        public Image CustomPawn1
        {
            get
            {
                return customPieceImages[0];
            }
            set
            {
                customPieceImages[0] = value;
            }
        }
        [DefaultValue(null)]
        public Image CustomKing1
        {
            get
            {
                return customPieceImages[1];
            }
            set
            {
                customPieceImages[1] = value;
            }
        }
        [DefaultValue(null)]
        public Image CustomPawn2
        {
            get
            {
                return customPieceImages[2];
            }
            set
            {
                customPieceImages[2] = value;
            }
        }
        [DefaultValue(null)]
        public Image CustomKing2
        {
            get
            {
                return customPieceImages[3];
            }
            set
            {
                customPieceImages[3] = value;
            }
        }
        ...
        public bool ShowJumpMessage
        {
            get
            {
                return showJumpMessage;
            }
            set
            {
                showJumpMessage = value;
            }
        }
        ...
        public CheckersPiece Holding
        {
            get
            {
                return ((IsHolding) ? (movePiece.Piece) : (null));
            }
        }
        ...
        public bool IsHolding
        {
            get
            {
                return (IsPlaying) && ((state == CheckersUIState.DragMoving) || (state == CheckersUIState.ClickMoving));
            }
        }
        ...
        public CheckersMove CurrentMove
        {
            get
            {
                return ((IsMoving) ? (movePiece.Clone()) : (null));
            }
        }
        ...
        public bool IsMovingByPlayer
        {
            get
            {
                return (IsPlaying) && ((state == CheckersUIState.DragMoving) || (state == CheckersUIState.ClickMoving) || (state == CheckersUIState.IdleMoving));
            }
        }
        ...
        public bool IsMovingByControl
        {
            get
            {
                return (IsPlaying) && (state == CheckersUIState.ShowMove);
            }
        }
        ...
        public int Winner
        {
            get
            {
                return ((game != null) ? (game.Winner) : (0));
            }
        }
        ...
        {
            if((state != CheckersUIState.DragMoving) && (state != CheckersUIState.ClickMoving) && (state != CheckersUIState.IdleMoving))
                return;
            if(movePiece == null)
                return;
        ...
        private void DoFocusSquare(Point location, bool refresh)
        {
            if(!IsPlaying)
                return;
            if(state == CheckersUIState.ShowMove)
                return;
            ...
            if(((game.Turn == 1) && (!player1Active)) || ((game.Turn == 2) && (!player2Active)))
                doHighlight = false;
            ...
            // Highlight board
            if((!focussedSquare.IsEmpty) && (focussedSquare == piece.Location))
                return;
        ...
        private void CheckersUI_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
        {
            ...
            switch(state)
            {
                ...
                case CheckersUIState.IdleMoving:
                    ...
                    if(piece.Player != game.Turn)
                        break;
                    if(movePiece.CurrentLocation != PointToGame(new Point(e.X, e.Y)))
                        break;
        ...
        private void tmrShowMove_Tick(object sender, System.EventArgs e)
        {
            ...
            if((movePiece.Path.Length < destPiece.Path.Length))
            {
                ...
                if(((!failed) && (movePiece.Path.Length < destPiece.Path.Length)) || (delayAfterShow))
                {
                    Refresh();
                    if(PieceMovedPartial != null)
                        PieceMovedPartial(this, new MoveEventArgs(false, false, movePiece.Clone()));
                    return;
                }
            }
            // Completed the path
            ...
            if((moveAfterShow) && (!failed))
                ...
                Refresh();
            if((PieceMoved != null) && (!delayAfterShow))
        ...
        public void ShowHint(CheckersAgent agent)
        {
            if((!IsPlaying) && (Winner == 0))
                return;
            if(!(((Game.Turn == 1) && (this.Player1Active)) || (Game.Turn == 2) && (this.Player2Active)))
                return;
            ...
            for(int y = 0; y < CheckersGame.BoardSize.Height; y++)
            {
                for(int x = 0; x < CheckersGame.BoardSize.Width; x++)
                {
                    ...
                    if(highlightPossibleMoves)
                        foreach(Point p in selectedSquares)
                            if((p.X == x) && (p.Y == y))
                            {
                    ...
                    if((blinkCount % 2) == 0)
                        foreach(Point p in darkenedSquares)
                            if((p.X == x) && (p.Y == y))
                            {
